...
static inline v4sf
lab_f_m(const v4sf x)
{
  const v4sf epsilon = v4sf_setall(216.0f/24389.0f);
  const v4sf kappa   = v4sf_setall(24389.0f/27.0f);

  // calculate as if x > epsilon : result = cbrtf(x)
  // approximate cbrtf(x):
  /* FIXME: Reoptimize based on SSE code */
  const v4sf a = v4sf_set(cbrtf(x[3]),cbrtf(x[2]),cbrtf(x[1]),cbrtf(x[0]));

  const v4sf a3 = a*a*a;
  const v4sf res_big = (a*(a3+x+x))/(a3+a3+x);

  // calculate as if x <= epsilon : result = (kappa*x+16)/116
  const v4sf res_small = ((kappa*x)+v4sf_setall(16.0f))/v4sf_setall(116.0f);
...
static inline v4sf
dt_XYZ_to_Lab_SSE(const v4sf XYZ)
{
  const v4sf d50_inv  = v4sf_set(0.0f, 1.0f/0.8249f, 1.0f, 1.0f/0.9642f);
  const v4sf coef = v4sf_set(0.0f,200.0f,500.0f,116.0f);
  const v4sf f = lab_f_m(XYZ*d50_inv);
  // because d50_inv.z is 0.0f, lab_f(0) == 16/116, so Lab[0] = 116*f[0] - 16 equal to 116*(f[0]-f[3])
  return coef*(__builtin_shuffle(f, v4si_set(3,1,0,1))-__builtin_shuffle(f, v4si_set(3,2,1,3)));
}
...
void process (struct dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, void *i, void *o, const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out)
{
  ...
  if(mat[0] != -666.0f)
  {
    ...
    for(int j=0; j<roi_out->height; j++)
    {
      ...
      float cam[3];
      const v4sf m0 = v4sf_set(0.0f,mat[6],mat[3],mat[0]);
      const v4sf m1 = v4sf_set(0.0f,mat[7],mat[4],mat[1]);
      const v4sf m2 = v4sf_set(0.0f,mat[8],mat[5],mat[2]);
      ...
      for(int i=0; i<roi_out->width; i++, buf_in+=ch, buf_out+=ch )
      {
        ...
        *XYZ = m0*cam[0]+m1*cam[1]+m2*cam[2];
        dt_XYZ_to_Lab(XYZ, buf_out);
#endif
        v4sf xyz = m0*cam[0]+m1*cam[1]+m2*cam[2];
        *(v4sf*)buf_out = dt_XYZ_to_Lab_SSE(xyz);
      }
    }
    vector_sfence();
  }
